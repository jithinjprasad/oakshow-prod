// CommonJS version for oakshow-prod-clean/index.js
const fs = require('fs');
const path = require('path');
const http = require('http');

const DIST_DIR = fs.existsSync('/app/dist')
  ? '/app/dist'
...
async function handler(req) {
  try {
    const rawUrl = typeof req === 'string'
      ? req
      : (req.url || req.path || req.rawUrl || '/');
...
if (require.main === module || process.env.STANDALONE_HTTP === 'true') {
